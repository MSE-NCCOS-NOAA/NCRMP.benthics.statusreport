## Function to calculate domain level z scores for old mortality
...
# DRM_SCREAM_calculate_mortality.R
# Site level data
...
NCRMP_calculate_Z_scores_mortality <- function(region, species_filter = "NULL", min_year_ref, max_year_ref, min_year_current, max_year_current, reference_value, std, reef_type){
  ...
  if(region == "SEFCRI"){
    ...
    dat_ref <- DRM_FL_2005_13_old_mort_site %>%
    ...
    NCRMP_SEFCRI_14_18_old_mort_site <- tmp$old_mortality_site

    dat_current <- NCRMP_SEFCRI_14_18_old_mort_site %>%
  ...
  if(region == "FLK"){
    ...
    NCRMP_FLK_14_18_old_mort_site <- tmp$old_mortality_site

    dat_current <- NCRMP_FLK_14_18_old_mort_site %>%
  ...
  if(region == "Tortugas"){
    ...
    NCRMP_Tort_14_18_old_mort_site <- tmp$old_mortality_site

    dat_current <-  NCRMP_Tort_14_18_old_mort_site %>%
  ...
  if(region == "STTSTJ"){
    ...
    # Calculate site level z scores for reference and current data
    dat_ref <- NCRMP_STTSTJ_2013_17_old_mort_site %>%
  ...
  if(region == "STX"){
    ...
    # Calculate site level z scores for reference and current data
    dat_ref <- NCRMP_STX_2015_17_old_mort_site %>%
  ...
  if(region == "GOM"){
    ...
    dat_ref <- FGBNMS_LTM_2017_old_mort_site %>%
      dplyr::filter(YEAR <= max_year_ref,
                    YEAR >= min_year_ref) %>%
      ...
      dplyr::bind_rows(NCRMP_FGBNMS_2013_18_old_mort_site %>%
                         dplyr::filter(YEAR <= max_year_ref,
                                       YEAR >= min_year_ref) %>%
    ...
    dat_current <- FGBNMS_LTM_2017_old_mort_site %>%
      dplyr::filter(YEAR <= max_year_current,
                    YEAR >= min_year_current) %>%
      dplyr::group_by(REGION, SURVEY, SUB_REGION_NAME, ADMIN, PRIMARY_SAMPLE_UNIT, LAT_DEGREES, LON_DEGREES, STRAT) %>%
      dplyr::summarise(avsitemort = mean(avsitemort)) %>%
